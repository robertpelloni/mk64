...
void destroy_banana_in_banana_bunch(struct BananaActor* banana) {
    ...
    if ((gPlayers[banana->playerId].type & PLAYER_HUMAN) != 0) {
        audio_play_sound_by_player_id(banana->playerId, SOUND_ARG_LOAD(0x19, 0x01, 0x90, 0x53));
    }
    banana->flags = -0x8000;
        ...
        case 6:
            ...
            if (someCount == 0) {
                ...
                if ((controller->buttonPressed & Z_TRIG) != 0) {
                    ...
                    audio_play_sound_by_player_id(owner - gPlayerOne, SOUND_ARG_LOAD(0x19, 0x00, 0x80, 0x12));
    ...
    switch (parent->state) {
        case SPAWN_FIRST_SHELL:
            if (init_triple_shell(parent, &gPlayers[playerId], shellType, 0U) != -1) {
                audio_play_sound_by_player_id(playerId, SOUND_ARG_LOAD(0x19, 0x00, 0x80, 0x12));
            ...
            if (parent->unk_08 > 0.0f) {
                if (parent->shellIndices[0] > 0.0f) {
                    ...
                    if ((shell->rotAngle < DEGREES(5)) || (shell->rotAngle > -DEGREES(5))) {
                        ...
                        audio_play_sound_by_player_id(parent->playerId, SOUND_ARG_LOAD(0x19, 0x00, 0x80, 0x04));
                        audio_play_character_sound(parent->playerId,
...
void func_802B2914(struct BananaBunchParent* banana_bunch, Player* player, s16 bananaId) {
    ...
    if (actorIndex >= 0) {
        ...
        if ((player->type & PLAYER_HUMAN) != 0) {
            audio_play_sound_by_player_id(player - gPlayerOne, SOUND_ARG_LOAD(0x19, 0x00, 0x80, 0x12));
        }
    }
}
